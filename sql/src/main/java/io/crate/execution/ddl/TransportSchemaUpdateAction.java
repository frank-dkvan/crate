    ...
    private CompletableFuture<SchemaUpdateResponse> updateTemplate(ImmutableOpenMap<String, IndexTemplateMetaData> templates,
        ...
        clusterService.submitStateUpdateTask("update-template-mapping", new ClusterStateUpdateTask(Priority.HIGH) {
            ...
            public ClusterState execute(ClusterState currentState) throws Exception {
                ClusterState newState = updateTemplate(xContentRegistry, currentState, templateName, newMapping);
                if (logger.isTraceEnabled()) {
                    if (currentState.metaData().version() < newState.metaData().version()) {
                        logger.trace("Template mapping update oldTemplateMapping={} newTemplateMapping={}",
                            currentState.metaData().templates().get(templateName).mappings(),
                            newState.metaData().templates().get(templateName).mappings());
                    } else {
                        logger.trace("Template mapping update without change; template={} newMapping={}",
                            currentState.metaData().templates().get(templateName).mappings(),
                            newMapping
                        );
                    }
